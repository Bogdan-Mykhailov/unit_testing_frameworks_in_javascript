...
export class NumbersValidator {
  ...
  isNumberEven(number) {
    ...
    if (typeOfVariable !== 'number') {
      ...
      // If it's a number, return true if it's even
      // (remainder of division by 2 is zero)
      return number % 2 === 0;
    }
  }
   ...
   * The array containing numbers to be checked
  ...
  getEvenNumbersFromArray(arrayOfNumbers) {
    ...
    if (Array.isArray(arrayOfNumbers) &&
      ...
      // Use the filter method to apply isNumberEven
      // on each element and return an array of even numbers
   ...
   * @return {Boolean} - Returns true if all elements are numbers,
   * false otherwise
   */
